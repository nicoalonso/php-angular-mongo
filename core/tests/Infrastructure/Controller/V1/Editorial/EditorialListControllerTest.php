<?php

namespace App\Tests\Infrastructure\Controller\V1\Editorial;

use App\Application\Editorial\List\EditorialList;
use App\Infrastructure\Controller\V1\Editorial\EditorialListController;
use App\Tests\Doubles\Infrastructure\Persistence\EditorialRepositoryStub;
use App\Tests\Infrastructure\ControllerTestable;
use PHPUnit\Framework\TestCase;
use Symfony\Component\HttpKernel\Exception\BadRequestHttpException;

class EditorialListControllerTest extends TestCase
{
    use ControllerTestable;

    private EditorialRepositoryStub $repository;
    private EditorialList $lister;
    private EditorialListController $controller;

    protected function setUp(): void
    {
        parent::setUp();

        $this->repository = new EditorialRepositoryStub();
        $this->lister = new EditorialList($this->repository);
        $this->controller = new EditorialListController();
    }

    public function testShouldFailWhenBadRequest(): void
    {
        $request = $this->createRequest([
            'test' => 'value',
        ]);

        $this->expectException(BadRequestHttpException::class);
        $this->controller->__invoke($request, $this->lister);
    }

    public function testShouldRunWhenDispatch(): void
    {
        $this->repository->attachAll();

        $request = $this->createRequest();
        $response = $this->controller->__invoke($request, $this->lister);

        $data = self::assertResponse($response);
        self::assertGreaterThanOrEqual(1, $data);
    }
}
