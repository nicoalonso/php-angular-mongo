...
final class CustomerListController
{
    public function __invoke(Request $request, CustomerList $lister): Response
    {
        try {
            $params = $request->query->all();
            $query = ListQuery::fromParams($params);
            $list = $lister->dispatch($query);
            $result = new ListView($list, CustomerReadView::class);
